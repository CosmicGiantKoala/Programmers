 ...
 */
    public class Exam468370 :Exam
    {
        private List<int[]> _ranges = new List<int[]>();
        private List<Word> _words = new List<Word>();
        private int[,] spoiler_range;
        public override void Run()
        {
            ...
            for (int range = 0; range < 1000; range++)
            {
                ...
                if (input2 == string.Empty) break;
                var min_range = int.Parse(input2.Split(',')[0]);
                var max_range = int.Parse(input2.Split(',')[1]);
                _ranges.Add(new int[2]{min_range,max_range});
            }
            spoiler_range = new int[_ranges.Count, 2];
            for (int i = 0; i < _ranges.Count; i++)
            {
                spoiler_range[i, 0] = _ranges[i][0];
                spoiler_range[i, 1] = _ranges[i][1];
            }

            int answer = 0;
            int spaceCount = message.Count(value => value == ' ');
            var words = message.Split(' ');
            
            _words.Add(new Word(words[0], 0, words[0].Length -1));
            var wordCount = words[0].Length;
            for (int i = 1; i < spaceCount+1; i++)
            {
                wordCount++;
                _words.Add(new Word(words[i], wordCount, wordCount + words[i].Length -1));
                wordCount = wordCount + words[i].Length;
            }

            foreach (var word in _words)
            {
                word.CheckSpoiler(spoiler_range);
            }
            ...
            foreach (var word in _words)
            {
                ...
                var compareList = new List<Word>(_words);
                compareList.Remove(word);
                compareList.RemoveAll(value => value.wordString != word.wordString);
                if (compareList.All(value => value.spoiler))
                {
                    answer++;
                    word.spoiler = false;
                }
        ...
        private class Word
        {
            ...
            public Word(string word, int startIdx, int endIdx)
            {
                wordString = word;
                _startIdx = startIdx;
                _endIdx = endIdx;
            ...
            public void CheckSpoiler(int[,] spoilerRange)
            {
                for (int i = 0; i < spoilerRange.GetLength(0); i++)
                {
                    if (Validate(_startIdx, _endIdx, spoilerRange[i, 0], spoilerRange[i, 1])) spoiler = true;
                }
            }
            private bool Validate(int wordStartIdx, int wordEndIdx, int spoilerStartIdx, int spoilerEndIdx) =>
                Math.Max(wordStartIdx, spoilerStartIdx) <= Math.Min(wordEndIdx, spoilerEndIdx);
        }
    }
}